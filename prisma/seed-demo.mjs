/**
 * Demo seed — plain ESM Node script (no TypeScript transpiler needed).
 * Uses the already-compiled Prisma client from src/generated/prisma.
 *
...
async function main() {
  ...
  for (const { symbol, prev, curr } of PAIRS) {
    ...
    await db.marketSnapshot.create({ data: { symbol, price: prev.price, volume: prev.volume, source: "demo", reliability: 0.9, timestamp: prev.ts } });
    await db.marketSnapshot.create({ data: { symbol, price: curr.price, volume: curr.volume, source: "demo", reliability: 0.9, timestamp: curr.ts } });
  ...
  for (const { symbol, type, headline, ts } of EVENTS) {
    const existing = await db.marketEvent.findFirst({ where: { symbol, type } });
    if (existing) { console.log(`  ⏭  Event ${symbol}/${type} already exists`); continue; }
    await db.marketEvent.create({ data: { symbol, type, headline, timestamp: ts } });
